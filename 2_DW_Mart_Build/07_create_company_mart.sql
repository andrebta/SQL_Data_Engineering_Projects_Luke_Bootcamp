...
-- Step 7.1: Create the mart schema

CREATE SCHEMA company_mart;

-- Step 7.2: Create dimension tables

-- Step 7.2.1: Company Dimension
...
-- Step 7.2.2a: Job title short dimension (distinct job_title_short values with IDs)
...
-- Step 7.2.2b: Job title dimension (distinct job_title values with IDs) -- Troquei por ROW_NUMBER() por questão de eficiência
...
--7.2.3: Location dimension (unique location/country combinations) - Novamente utilizei o ROW_NUMBER() para realizar a criação do location_id
...
--7.2.4: Month-level date dimension - Adicionei um ORDER BY para facilitar a leitura
...
--7.2.5: Bridge table: Company to location (many-to-many) 
-- Confusão por que eu achei que company_dim do main já contava com o dado de local da empresa, mas não, precisei tirar essa informação da jpf
...
--7.2.6: Bridge table: Job Title Short to Job Title (many-to-many)
...
-- 7.3: Create fact table - fact_company_hiring_monthly
-- Grain: company_id + job_title_short_id + job_contry + posted_month
